...
| 4 | PA8 | UART1_TX |  |
| 5 | PA26 | OPA0_IN0+、ADC0.1 | 当前固件作外部电路控制线（测量时拉低），必须断开 J18 |
...
| J4 | 已安装 | PA0 连接红色 LED1 | 当前固件的上电指示灯占用 PA0，3 Hz 闪烁 |
...
| — | 已贴 | C8 = 1 µF 接在 PA23/VREF+ 与 VREF- 之间；R3 = 0 Ω 把 PA21/VREF- 接地 | 内部 2.5 V 基准所需的 CVREF，开发板自带，无需改动。通往排针的 R20/R22 均为 DNC，两个 VREF 脚不引出，外部不会挂负载。换自制板时这两个元件必须照抄——数据手册 7.15.2 注 5：没有 CVREF 就不得使能 VREF 模块 |
...
| J18 | 已安装 | PA26 连接光传感器电路 | 当前固件把 PA26 当输出驱动，必须断开，否则与光传感器电路对打 |
...
- 地址开关使用 PB0、PB6、PB7、PB8 后，会占用 SPI1 的 CS/POCI/PICO 引脚，不能同时使用当前 ADS131A04 固定驱动；PB6 也与 ILI9341、AD9910、AD9959 冲突。
...
| 功能 | 外设/引脚 | 状态 |
| --- | --- | --- |
| 串口发送指示绿灯 | PB27 / RGB 绿灯 | 当前固件已使用 |
| 报警红灯 | PA0 或 PB26 / RGB 红灯 | 预留 |
| OLED 128×64 | PB2 SCL、PB3 SDA | 计划使用 SSD1306 固定驱动 |
| 模拟前端 | OPA1，PB19 输入、PA16 输出或内部 ADC 通路 | 待模拟链路实测确认 |
| HC-42 TXD | PB16 / UART2_RX | 推荐 |
| HC-42 RXD | PB15 / UART2_TX | 推荐。波特率 115200，非出厂的 9600——模块须先用 `cargo run --bin btcfg` 改过，两边不一致时模块是沉默而不是报错 |
| HC-42 电源开关 | PB17 / GPIO | 驱动外部高电平有效负载开关的 EN，不得直接驱动 VCC；启用 AD9910/AD9959 时存在引脚冲突 |
| HC-42 电源 | 3V3 经负载开关后接 VCC、GND | 禁止接 5V；负载开关输出能力不小于 50 mA |
| 地址开关 S1 / bit 0 | PB0，排针 12 | 权重 1，ON 接地 |
| 地址开关 S2 / bit 1 | PB6，排针 13 | 权重 2，ON 接地 |
| 地址开关 S3 / bit 2 | PB7，排针 14 | 权重 4，ON 接地 |
| 地址开关 S4 / bit 3 | PB8，排针 15 | 权重 8，ON 接地 |
...
| 3V3 | 外部负载开关 VIN |
| J2/J4-18，PB17/GPIO | 外部负载开关 EN |
| 外部负载开关 VOUT | VCC |
| GND | GND |

### 6.1 HC-42 启动功耗控制

HC-42 的 VCC 不再直接连接开发板 3V3，而是由外部高电平有效的 3.3 V
负载开关控制：
...
PB17 ─> 负载开关 EN
         负载开关 VOUT ──> HC-42 VCC
GND  ────────────────────> HC-42 GND
PB15 / UART2_TX ─────────> HC-42 RXD
PB16 / UART2_RX <───────── HC-42 TXD
```

- EN 必须增加外部下拉电阻，使 MCU 复位、烧录或尚未接管 PB17 时 HC-42
  保持断电。建议下拉电阻取 `47 kΩ～100 kΩ`。
- 负载开关须支持 3.3 V，持续输出能力不小于 HC-42 手册要求的 `50 mA`。
- PB17 只连接负载开关 EN，**不得使用 GPIO 直接给 HC-42 VCC 供电**。
- PB17 在当前主固件中没有其他用途，但 AD9910/AD9959 固定驱动会使用该
  引脚；当前工程未启用这两个 DDS 驱动，可以用于蓝牙电源控制。
- HC-42 断电期间不能提前初始化 UART2，否则 PB15 的空闲高电平可能经
  HC-42 RXD 保护二极管反向灌电。
- 当前固件上电后立即将 PB17 拉低，完成首帧采样、计算及 OLED 首次刷新后
  才将 PB17 拉高；随后等待 `350 ms`，再初始化 UART2 并发送首帧。
- 蓝牙启动后保持供电，以便手机或读表器随时扫描和连接。若再次断电，远端
  无法通过无线方式唤醒它，必须由本地按键、定时策略或其他硬件唤醒源重新
  拉高 PB17。

HC-42 的 `LPIN`（模块 19 脚）是低功耗控制脚，不是真正的电源开关。配合
`AT+PM=1` 可以使模块上电进入低功耗状态，但广播或连接状态仍可保持；若目标
是消除启动阶段的模块启动电流，应采用上述 VCC 负载开关方案。

4 位编码开关使用 GPIO 内部上拉，不需要外接上拉电阻。每个开关的一端分别连接 PB0、PB6、PB7、PB8，另一端全部连接 GND：
