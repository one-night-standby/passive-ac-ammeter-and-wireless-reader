...
探测帧长            & 160 点（40\,ms）\\
ADC                 & 12 位，参考 $V_{DDA}/V_{SSA}$，满量程 $0\text{--}4095$\\
...
\subsection{通信协议}
电流表每测量周期发送一帧 ASCII 数据，字段见表~\ref{tab:frame}，以回车换行结尾。接收端按行解析
并作格式校验，不合格式的行丢弃。电流表不接收控制命令。
...
字段 & 示例 & 说明\\
\midrule
帧头       & \texttt{METER\_TEST} & 固定标识\\
地址       & \texttt{ADDR=01}     & 四位编码开关，$0\text{--}15$\\
电流       & \texttt{CURRENT\_MA=1305} & 有效值，单位 mA\\
状态       & \texttt{STATUS=NORMAL}    & 状态标识\\
\bottomrule
\end{tabular}
\end{table}

\subsection{读表器软件}
读表器周期扫描（8\,s 扫描、12\,s 间歇），对搜索到的电流表分别建立连接并持续接收数据帧。基本
模式为一对一手动读取，每次操作记录一帧；自动模式一键启动，每 2 分钟记录各表最新一帧，1.5\,s
内无新数据的电流表记为离线。报警按电流值判定：小于 0.2\,A 为低限、大于 2\,A 为超限、无数据为
离线。记录存入手机内部数据库，含读取时间、电流表地址、电流值与状态，掉电不丢失，容量 1000 条，
并按地址绘制电流趋势曲线。
