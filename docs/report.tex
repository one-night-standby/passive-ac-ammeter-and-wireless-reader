...
随后不等请求先完成一次测量——读表器一连上就有数可读。这次测量结束后才把 PB17 置高、
等待 350\,ms 并初始化 UART2：模块的启动电流是本固件中最大的单笔负载，不应落在供电
刚建立的时刻。
...
\begin{tikzpicture}[scale=0.80,transform shape,font=\wuhao,
  ...
  \node[lb] (b2) at (0,-1.65)  {上电自测一次\\ 执行测量子流程};
  \node[lb] (b3) at (0,-3.30)  {PB17=1，等 350\,ms\\ 初始化 UART2};
  \node[lb] (b4) at (0,-4.95)  {发读数帧与调试帧\\ OLED 显示 1\,s 后熄屏};
  \node[lb] (b5) at (0,-6.60)  {清空积压请求，等待触发};
  \node[decision] (d6) at (0,-8.40) {触发来源？};
  \node[lb,text width=2.8cm] (b7) at (-5.20,-8.40)
    {发 \texttt{IMHERE} 保活帧};
  \node[lb] (b8) at (0,-10.15) {执行测量子流程};
  ...
  \draw[line] (b7.north)--(-5.20,-6.60)--(b5.west);
  \draw[line] (d6.south)--node[right]{S2 或 \texttt{MEAS}}(b8.north);
  \draw[line] (b8.east)--(3.90,-10.15)--(3.90,-4.95)--(b4.east);
  \draw[line] (m0)--(m1);
  \draw[line] (m1)--(m2);
  \draw[line] (m2)--(m3);
  \draw[line] (m3)--(m4);
